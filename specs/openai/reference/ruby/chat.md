...
- `web_search_options: { search_context_size, user_location}`
  ...
  - `user_location: { approximate, type}`
...
`chat.completions.list(**kwargs) -> CursorPage<ChatCompletion>`
...
`chat.completions.retrieve(completion_id) -> ChatCompletion`
...
`chat.completions.update(completion_id, **kwargs) -> ChatCompletion`
...
`chat.completions.delete(completion_id) -> ChatCompletionDeleted`
...
- `ChatCompletionContentPart = ChatCompletionContentPartText | ChatCompletionContentPartImage | ChatCompletionContentPartInputAudio | { file, type}`
