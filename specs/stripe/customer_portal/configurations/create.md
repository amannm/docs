...
Possible enum values:
  ...
  - `features.payment_method_update` (object, optional)
    ...
    - `features.payment_method_update.payment_method_configuration` (string, optional)
      The [Payment Method Configuration](../../payment_method_configurations.md) to use for this portal session. When specified, customers will be able to update their payment method to one of the options specified by the payment method configuration. If not set or set to an empty string, the default payment method configuration is used.
