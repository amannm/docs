...
A dictionary with a `data` property that contains an array of up to `limit` accounts, starting after account `starting_after`. Each entry in the array is a separate [`Account`](list.md#account_object) object. If no more accounts are available, the resulting array is empty.
