...
The updated webhook endpoint object if successful. Otherwise, this call raises [an error](update.md#errors).
...
Possible enum values:
  ...
  - `application_fee.refunded`
    Occurs whenever an application fee is refunded, whether from refunding a charge or from [refunding the application fee directly](update.md#fee_refunds). This includes partial refunds.
  ...
  - `customer.subscription.resumed`
    Occurs whenever a customer’s subscription is no longer paused. Only applies when a `status=paused` subscription is [resumed](../subscriptions/resume.md), not when [payment collection](https://docs.stripe.com/billing/subscriptions/pause.md) is resumed.
  ...
  - `invoice.deleted`
    Occurs whenever a draft invoice is deleted. Note: This event is not sent for [invoice previews](../invoices/create_preview.md).

  - `invoice.finalization_failed`
    Occurs whenever a draft invoice cannot be finalized. See the invoice’s [last finalization error](../invoices/object.md#invoice_object-last_finalization_error) for details.
  ...
  - `payment_intent.amount_capturable_updated`
    Occurs when a PaymentIntent has funds to be captured. Check the `amount_capturable` property on the PaymentIntent to determine the amount that can be captured. You may capture the PaymentIntent with an `amount_to_capture` value up to the specified amount. [Learn more about capturing PaymentIntents.](../payment_intents/capture.md)
  ...
  - `payment_method.updated`
    Occurs whenever a payment method is updated via the [PaymentMethod update API](../payment_methods/update.md).
