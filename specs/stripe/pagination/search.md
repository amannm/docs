...
Some top-level API resource have support for retrieval via “search” API methods. For example, you can [search charges](../charges/search.md), [search customers](../customers/search.md), and [search subscriptions](../subscriptions/search.md).
...
Our client libraries offer [auto-pagination](auto.md) helpers to easily traverse all pages of a search result.
...
- `total_count` (optional positive integer or zero)
  The total number of objects that match the query, only accurate up to 10,000. This field isn’t included by default. To include it in the response, [expand](../expanding_objects.md) the `total_count` field.
