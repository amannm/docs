...
Returns the created `Subscription Item` object, if successful. Otherwise, this call raises [an error](create.md#errors).
...
- `price_data` (object, optional)
  ...
  - `price_data.product` (string, required)
    The ID of the [Product](../products.md) that this [Price](../prices.md) will belong to.
...
- `proration_date` (timestamp, optional)
  If set, the proration will be calculated as though the subscription was updated at the given time. This can be used to apply the same proration that was previewed with the [upcoming invoice](create.md#retrieve_customer_invoice) endpoint.
